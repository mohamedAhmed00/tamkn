    ...
    <div class="col-lg-12">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ !empty($sliderItem)? ' تعديل الوثيقة ' : ' انشاء وثيقة جديدة ' }}</li>
            </ol>
        </nav>
        <div class="card">
            <div class="card-body">
                <form action="{{ !empty($sliderItem)? route('auth.lesson.sliderItem.update',[$sliderItem->id,$lesson->id]) : route('auth.lesson.sliderItem.store',$lesson->id) }}" method="post" novalidate="novalidate" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="tab-pane fade {{ $loop->iteration == 1 ? ' show active ' : '' }}" id="{{ $language->key }}" role="tabpanel" aria-labelledby="{{ $language->key }}_tab">
                                <div class="form-group">
                                    ...
                                    <input id="title_{{ $language->key }}" name="title_{{ $language->key }}" class="form-control" aria-required="true" autocomplete="none"
                                           aria-invalid="false" value="{{ empty(old('title_'.$language->key))? (!empty($sliderItem) AND !empty($sliderItem->getSliderItemDescription->where('language_id',$language->id)->first()))? $sliderItem->getSliderItemDescription->where('language_id',$language->id)->first()->title : ''  : old('title_'.$language->key) }} " type="text">
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" id="status" class="form-control-sm form-control">
                            ...
                            <option value="0" {{ empty(old('status'))? (!empty($sliderItem) AND $sliderItem->status == '0')?'selected':''  : 'selected' }}>تعطيل</option>
                            <option value="1" {{ empty(old('status'))? (!empty($sliderItem) AND $sliderItem->status == '1')?'selected':''  : 'selected' }}>تفعيل</option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input id="order" name="order" class="form-control" aria-required="true" autocomplete="none"
                               aria-invalid="false" value="{{ empty(old('order'))? !empty($sliderItem)?$sliderItem->order:''  : old('order') }}" type="number">
                    </div>
