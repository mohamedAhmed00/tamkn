    ...
    <div class="col-lg-12">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ !empty($part)? ' تعديل الجزء '  : ' انشاء جزء جديد ' }}</li>
            </ol>
        </nav>
        <div class="card">
            <div class="card-body">
                <form action="{{ !empty($part)? route('auth.course.part.update',[$part->id,$course->id]) : route('auth.course.part.store',$course->id) }}" method="post" novalidate="novalidate" enctype="multipart/form-data">
                    @csrf
                        ...
                        @foreach($languages as $language)
                            <div class="tab-pane fade {{ $loop->iteration == 1 ? ' show active ' : '' }}" id="{{ $language->key }}" role="tabpanel" aria-labelledby="{{ $language->key }}_tab">
                                <div class="form-group">
                                    <label for="title_{{ $language->key }}"  class="control-label mb-1">اسم الجزء باللغه : {{ $language->name }} </label>
                                    <input id="title_{{ $language->key }}" name="title_{{ $language->key }}" class="form-control" aria-required="true" autocomplete="none"
                                           aria-invalid="false" value="{{ (!empty($part) AND !empty($part->getPartDescription->where('language_id',$language->id)->first()))? $part->getPartDescription->where('language_id',$language->id)->first()->title : '' }}" type="text">
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" id="status" class="form-control-sm form-control">
                            ...
                            <option value="0" {{ empty(old('status'))? (!empty($part) AND $part->status == '0')?'selected':''  : 'selected' }}>تعطيل</option>
                            <option value="1" {{ empty(old('status'))? (!empty($part) AND $part->status == '1')?'selected':''  : 'selected' }}>تفعيل</option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input id="order" name="order" class="form-control" aria-required="true" autocomplete="none"
                               aria-invalid="false" value="{{ empty(old('order'))? !empty($part)?$part->order:''  : old('order') }}" type="number">
                    </div>
